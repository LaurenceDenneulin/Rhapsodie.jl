...
function apply_rhapsodie(x0::TPolarimetricMap, A::D, d::Array{data_table,1}, par::Array{T,1}; mem=3, maxeval=50, maxiter=50, xtol=(1e-3,1e-8), gtol=(1e-3,1e-8), ftol=(1e-3,1e-8)) where {T <: AbstractFloat, D <:Mapping}
    n1,n2 = size(x0)
...
function apply_gradient!(X::TPolarimetricMap, A::D, g::Array{T,3}, d::Array{data_table,1}, μ::Array{hyperparameters{T},1}) where {T <: AbstractFloat, D <:Mapping}
    ...
    if X.parameter_type == "mixed"
        ...
        f+=apply_edge_preserving_smoothing!(cat(X.Iu_disk[:,:], X.Q[:,:], X.U[:,:], dims=3), view(g,:,:,2:3), μ[2].λ, μ[2].ρ)
     elseif X.parameter_type == "stokes"
 	    f+=apply_edge_preserving_smoothing!(X.I_star[:,:], view(g,:,:,1), μ[1].λ, μ[1].ρ);
        f+=apply_edge_preserving_smoothing!(cat(X.I_disk[:,:], X.Q[:,:], X.U[:,:], dims=3), view(g,:,:,2:3), μ[2].λ, μ[2].ρ)
